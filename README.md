<p align="center">
  <img src="assets/banner.png" alt="QUSX â€” Unified Scripture-style Markup for the Qur'an. One text, many layers, zero duplication." width="100%">
</p>

<p align="center">
  <a href="https://dfordev1.github.io/usxv2/app/"><img alt="Live demo" src="https://img.shields.io/badge/â–¶_live_demo-open_the_app-3f7d4f.svg"></a>
...
<p align="center">
  <b>â–¶ Try it live: <a href="https://dfordev1.github.io/usxv2/app/">dfordev1.github.io/usxv2/app</a></b> â€” read the Mushaf, compare traditions, and inspect any word, straight in the browser. No install.
</p>

# QUSX â€” a milestone-based XML standard for the Qur'an
...
Qur'anic text inside a rigid surah â†’ ayah â†’ word tree, the Mushaf is treated as **one
...
layouts can coexist over that *same* text stream â€” without duplicating a single
letter.
...
Library / USX ecosystem solved the same problem â€” Scripture text served consistently
across 1,773 languages â€” for Bible software.
...
The npm package contains the parser, TypeScript declarations, and schemaâ€”not the 98 MB generated corpus. The client fetches individual surahs on demand; production users can point it at their own versioned CDN. See the [SDK guide](docs/sdk.md) for parsing, local-file, and query examples.
...
Mushaf), one for searchable/structured text, one for word-level morphology, and â€” for
apps supporting more than one riwayah â€” a full duplicate text per riwayah. Every
...
needs within that surah â€” an ayah, a printed page, a juz fragment, a riwayah's
numbering â€” without re-fetching or duplicating the underlying text *within that
file*. To be precise about current scope: this project ships **1140 files** (114
surahs Ã— 10 layouts), and text/morphology *is* duplicated once per layout â€” a page-2
...
regenerating the underlying text) is unresolved â€” see [Known gaps](#known-gaps).
...
text) â€” only page/line placement changes per print edition, and each IndoPak/Nastaleeq
...
numbers run 1..N with no gaps) â€” **all 1140 files pass, 0 errors.** Separately,
`scripts/xsd_validate.py` runs the actual [`schema/qusx.xsd`](schema/qusx.xsd)
through a real XML Schema processor (`lxml`/libxml2) â€” **all 1140 files are also
...
XSD-valid ayah-numbering pins with real Arabic text for 5 more traditions â€” Warsh,
Qalun, Al-Duri, Shu'bah and Al-Susi (570 files, `output-pilot/`). Together with
Hafs, the research prototype covers six riwÄyÄt. This is genuinely real, not a demo: real per-tradition text,
verified to differ from Hafs at the rasm level, not a relabeled copy. It is
**deliberately narrower** than the main generator's output, though â€” no
...
### Six-riwÄyah alignment prototype
...
textual differences across all six riwÄyÄt. It uses stable slot IDs without
...
`createAyahMappingClient()` for cross-riwÄyah ayah navigation. The generated
...
subtyping remains explicitâ€”reviewed where supported, otherwise
...
Al-Bazzi, Qunbul and Al-Susi PDFs. The conservative result is 216 reading
variants, 87 presentation differences, 6 tokenization cases and 624 uncertain
...
```xml
<qusx version="0.1" surah="1" name="Al-Fatihah" nameArabic="Ø§Ù„ÙØ§ØªØ­Ø©" ayahCount="7"
  ...
  <word id="1" position="1" root="Ø³   Ù…   Ùˆ" stem="Ø³Ù’Ù…Ù" lemma="Ø§Ø³Ù’Ù…">Ø¨ÙØ³Ù’Ù…Ù</word>
  <word id="2" position="2" root="Ø§ Ù„ Ù‡" stem="Ø§Ù„Ù„ÙŽÙ‘Ù‡Ù" lemma="Ø§Ù„Ù„ÙŽÙ‘Ù‡">Ù±Ù„Ù„ÙŽÙ‘Ù‡Ù</word>
  ...
...
matching `sid`/`eid` pairs over the same underlying word stream â€” no duplicated text
...
| Tag | Role | Notes |
|---|---|---|
| `<qusx>` | Root, one per surah | `tradition` = active ayah-counting scheme; `normalization="NFC"` states the text-encoding policy (checked by `validate.js`); `generatorVersion` self-declares what produced the file; `bismillahPre` â€” see note below |
| `<word>` | Base text unit (leaf) | `id` = global mushaf position, `position` = index within ayah, `type="number"` marks the ayah-ending verse-number glyph (not a lexical word â€” see below), `root`/`stem`/`lemma` = morphology embeds |
| `<ayah>` | Milestone pin | `tradition` attr allows multiple counting schemes over the same word stream |
| `<page>` / `<line>` | Milestone pin | From a specific print edition's layout â€” see `<qusx layout="...">` on the root element |
| `<juz>` / `<hizb>` / `<rub>` / `<manzil>` / `<ruku>` | Milestone pin | Standard 30/60/240/7-way divisions, plus thematic ruku markers (558 total); `fragment` (`whole`/`start`/`middle`/`end`) states whether this file's copy is the entire range or a piece of one spanning surahs â€” see below. Definitions in [`docs/quranic-structural-glossary.xlsx`](docs/quranic-structural-glossary.xlsx) |
| `<sajda>` | Point marker (non-paired) | Fires once at the ayah containing a prostration point; `type` = `required`/`optional`. **Precision limit, verified not fixable with current data:** QUL's sajda dataset only gives `verse_key` (ayah-level), not a word position â€” checked directly against `data/raw/quran-metadata-sajda.json`, which has no word field at all. The pin fires after the ayah's last word as the closest available approximation; pinning to the exact word would need a different/richer data source. |

**What `bismillahPre` means â€” verified, not assumed:** it's a **display-only flag**,
not a pointer to any `<word>` elements. Checked directly against real data: Surah
2's ayah 1 is just `Ø§Ù„Ù“Ù…Ù“` (Alif-Lam-Meem) â€” no Basmalah words appear in the word
...
a surah's name), not part of the counted ayah text â€” except in Al-Fatihah, where
it genuinely *is* ayah 1 (and `bismillahPre="false"` there for exactly that
reason â€” it's already inside the ayah stream, not a separate heading), and
...
own â€” QUSX doesn't carry it as data, and doing so would either duplicate the
...
well-paired â€” but axes are **not** required to nest inside one another in document
order. `page`/`line` are word-position-based and legitimately cross `ayah` (and
sometimes `juz`/`hizb`/`rub`) boundaries mid-ayah â€” e.g. SÅ«rat An-NÄs ayah 3 is split
...
Surah 1 and part of Surah 2) is written once per file it touches â€” `sid="juz:1"`
...
`002.qusx.xml`'s carries `fragment="end"` (continuation, ends here) â€” so a consumer
...
not one flat label â€” `hafs` names the **recitation transmission** (riwÄyah, i.e.
whose reading of NafiÊ¿/Ê¿Ä€á¹£im/etc. this is) and `kufi` names the **ayah-counting
system** (Ê¿add al-Äy school) applied to it. These are two genuinely independent
axes â€” e.g. Warsh's transmission can in principle be counted by more than one
school â€” and the schema's closed `traditionCode` enum (see `schema/qusx.xsd`)
...
- **[QUL](https://qul.tarteel.ai)** (TarteelAI) â€” Uthmani word-by-word text, ayah/juz/hizb/rub/ruku/manzil/sajda metadata, word root/stem/lemma (`data/raw/`), and 5 Mushaf layouts â€” KFGQPC V1/V2/V4-tajweed, Mushaf Qatar, IndoPak 15-line (`data/layouts/`).
- **[quranpedia/quran-svg](https://github.com/quranpedia/quran-svg)** â€” per-surah ayah counts across 6 mushaf editions/5 qira'at, used to derive multi-tradition boundary deltas. Our derived analysis is in `data/diff-report.json`; the source repo's raw files are *not* bundled here since it currently has no published license â€” see it directly if you need the underlying polygon/page data.
...
sequencing, required attributes by convention) â€” it is **not** an XSD validator and
...
`validate.js`, not by the schema itself â€” this is a real, stated scope split
...
â€” reads the full Mushaf across all 10 layouts, compares numbering traditions
...
to see two real generated files (Al-FÄtiá¸¥ah and An-NÄs) parsed live and
...
node src/checksum-verify.js                    # (superseded framing, kept for history â€” see below)
...
recorded *which* combination in its own metadata â€” so a QUL convention that
...
- Tanzil's **default export** (all 5 options off) + its own per-surah
  ...
  manifest is a valid Tanzil export â€” just an undocumented one, not a
  different/wrong text.
- Tanzil's **full-options export** (all 5 options on) â€” the configuration
  QUL's own convention actually matches â€” compared directly to QUL's raw
...
|---|---|---|
| 5:52 | formatting | QUL has a stray space inside a word: `Ø¯ÙŽØ¢Ø¦ÙØ±ÙŽ Ø©ÙŒÛ­` vs `Ø¯ÙŽØ¢Ø¦ÙØ±ÙŽØ©ÙŒÛ­` |
...
| 11:13 | **orthographic, needs review** | QUL: `Ø§ÙÙ’ØªÙŽØ±ÙŽØ§Ù‡Ù` (plain alef). Tanzil: `Ù±ÙÙ’ØªÙŽØ±ÙŽÙ‰Ù°Ù‡Ù` (alef wasla + alef maksura/superscript alef) â€” a different encoded letter, not spacing |
| 80:25 | **orthographic, needs review** | QUL: `Ø§ÙŽÙ†ÙŽÙ‘Ø§` (plain alef). Tanzil: `Ø£ÙŽÙ†ÙŽÙ‘Ø§` (alef with hamza above) â€” a different encoded letter, not spacing |

**Cross-checked against a third source, 2026-07-13:** fetched all 6 verse keys live
from the [quran.com v4 API](https://api.quran.com/api/v4/quran/verses/uthmani) â€”
...
these 6 positions â€” consistent with real QUL transcription defects rather than
a legitimate alternate convention.

**A transparent correction layer, not a silent edit, 2026-07-13:** the 4
formatting cases are now corrected â€” but only in a dedicated, committed,
...
comparison. **`data/raw/uthmani.json` itself is never modified** â€” it's
...
the live source on every run â€” if the underlying data ever changes, the
...
The 2 orthographic cases (11:13, 80:25) are deliberately left uncorrected â€”
...
QUL, not yet filed â€” it needs a human decision on the right channel).

The four "formatting" cases are QUL source-data encoding anomalies (extra/missing
whitespace, a stray bidi control character) â€” they don't change a single Quranic
...
data â€” see [`data/LICENSES.md`](data/LICENSES.md) â€” not something this project
...
but still committed) used Al-FÄtiá¸¥ah 1:1 as *the* Bismillah for every applicable
...
with shadda on the bÄ' (`Ø¨ÙÙ‘Ø³Ù’Ù…Ù...`, not `Ø¨ÙØ³Ù’Ù…Ù...`) â€” confirmed directly in
...
<details>
<summary>Superseded framing (kept for history, not deleted â€” the analysis
...
against that specific export configuration â€” expected, now that the configuration
...
explained before this investigation â€” a real result, but one aimed at
...
here â€” recorded rather than smoothed over.
...
1. **Multi-tradition text/alignment exists as a real research prototype, not yet in the main generator.**
   ...
   (and Al-Susi as a bonus) â€” sourced from `thetruetruth/quran-data-kfgqpc`, a
   third-party mirror of King Fahd Complex's official font/data packages â€”
   ...
   - **License status is a judgment call, not a confirmed clearance.** The
     ...
     extend to this derived text â€” see `THIRD_PARTY_NOTICES.md` for the full
     caveat. Treat as unverified-but-included, not cleared.
   - **Numbering/text only, not full parity with the main generator's output**
     â€” no morphology (root/stem/lemma), and no page/juz/hizb/rub/manzil/ruku
   ...
   - **Al-Duri has an unresolved 3-way count discrepancy** (6205 in the
     ...
     source) â€” precisely located to surah 67 (30 vs 31 ayahs). Narrowed, not
     resolved: this is a real, documented classical Ê¿add-al-Äy
     (verse-counting) disagreement â€” Al-Mulk has 30 ayahs under the
     majority/Kufi counting school and 31 under the á¸¤ijÄzÄ« school, not a
     data-quality bug. Al-Duri and Al-Susi are both transmitters of Abu Ê¿Amr
     al-Basri, so the true count should follow the classical Basri counting
     school specifically â€” that number lives in specialized Ê¿ulÅ«m al-QurÊ¾Än
     references (e.g. al-Dani's *al-Bayan fi Ê¿Add Ay al-Qur'an*), not
   ...
   **Current scope:** core QUSX XML remains numbering/milestone-oriented. A
   ...
   three authenticated locations and is not a complete qirÄÊ¾Ät corpus.
   **Checked and ruled out** as a text source: [tanzil.net/download](https://tanzil.net/download/)
   â€” verified directly, no riwayah/qira'a selector at all.
2. **10 of QUL's 12 print layouts are wired in** (KFGQPC V1/V2/V4-tajweed,
   ...
   but **not because of a data-shape mismatch** â€” that was a prior
   ...
   QUL session â€” not yet done, but should follow the same pattern as the
   other 10 layouts once downloaded.
3. **Text/morphology are duplicated once per layout**, not factored into a
   separately-referenceable layer â€” see the [Why this exists](#why-this-exists)
...
4. **2 of the 6,236 verses (11:13, 80:25) have a genuine, unresolved orthographic
   difference from an independent Tanzil export** â€” see
   ...
   claimed this gap was in the thousands (541, then 3,741, then 3,040) â€” all
...
5. **Only ~26 of the ~150+ items from the fuller third-party review have been
   triaged and fixed** â€” the rest (richer schema semantics beyond what's listed
...
6. **Sajda can't be pinned to an exact word with current data** â€” checked
...
check â€” committed output must exactly match a fresh build from source, verified with
...
actually reject/catch bad input and match known-real baselines â€” not just pass good
...
args deduplicated, not rejected â€” `node src/generate.js 1 1` exits 0 and
...
finished or formally released standard â€” see the
...
```
qusx/
â”œâ”€â”€ .github/workflows/ci.yml   # runs validate.js + xsd_validate.py + test/ on every push
â”œâ”€â”€ src/
â”‚   â”œâ”€â”€ generate.js           # the generator
â”‚   â”œâ”€â”€ validate.js           # semantic conformance checker (not an XSD validator)
â”‚   â””â”€â”€ checksum-verify.js    # cross-checks text against an independent SHA-256 manifest
â”œâ”€â”€ scripts/
â”‚   â”œâ”€â”€ xsd_validate.py       # real XSD validation via lxml (requirements.txt)
â”‚   â””â”€â”€ build_glossary_xlsx.py
â”œâ”€â”€ test/
â”‚   â”œâ”€â”€ run_tests.js          # negative tests: proves validators actually reject bad input
â”‚   â””â”€â”€ fixtures/             # deliberately malformed/invalid .qusx.xml files
â”œâ”€â”€ schema/qusx.xsd            # formal element/attribute shape
â”œâ”€â”€ viewer/viewer.html         # live DOMParser-based viewer (self-contained)
â”œâ”€â”€ data/
â”‚   â”œâ”€â”€ raw/                  # QUL exports (text, metadata, morphology, default layout)
â”‚   â”œâ”€â”€ layouts/               # 4 additional QUL Mushaf layout databases
â”‚   â”œâ”€â”€ external/              # third-party manifests (quranchecksum)
â”‚   â””â”€â”€ diff-report.json      # our derived per-surah ayah-count deltas across traditions
â”œâ”€â”€ output/<layout-key>/      # generated *.qusx.xml, one per surah per layout
â”œâ”€â”€ assets/                   # banner and diagram images
â”œâ”€â”€ LICENSE                   # MIT (code/schema only â€” see note on bundled data)
â”œâ”€â”€ THIRD_PARTY_NOTICES.md     # exact provenance/terms for every bundled dataset
â”œâ”€â”€ CONTRIBUTING.md
â”œâ”€â”€ CHANGELOG.md
â”œâ”€â”€ requirements.txt           # Python deps for scripts/
â”œâ”€â”€ package.json
â””â”€â”€ docs/
    â”œâ”€â”€ prior-art-references.md
    â””â”€â”€ quranic-structural-glossary.xlsx
```
