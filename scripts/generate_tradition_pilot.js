...
//
// Usage: node scripts/generate_tradition_pilot.js <tradition> <surahNumber>
...
const SURAH = Number(process.argv[3]);
if (!TRADITION || !SURAH) {
  console.error("Usage: node scripts/generate_tradition_pilot.js <tradition> <surahNumber>");
  process.exit(1);
}
...
function main() {
  ...
  const surahAyahs = allAyahs.filter((e) => e.sura_no === SURAH).sort((a, b) => a.aya_no - b.aya_no);
  if (surahAyahs.length === 0) throw new Error(`No ayahs found for surah ${SURAH} in ${textFile}`);

  const canonical = surahNames[String(SURAH)];
  if (!canonical) throw new Error(`No canonical surah metadata found for surah ${SURAH}`);

  const lines = [];
  lines.push('<?xml version="1.0" encoding="UTF-8"?>');
  lines.push(
    `<qusx version="0.1" surah="${SURAH}" name="${xmlEscape(normalize(canonical.name_simple))}" ` +
  ...
  for (const ayahRec of surahAyahs) {
    const verseKey = `${SURAH}:${ayahRec.aya_no}`;
  ...
  const outPath = path.join(OUT_DIR, `${String(SURAH).padStart(3, "0")}.qusx.xml`);
  fs.writeFileSync(outPath, xml, "utf-8");
  console.log(`wrote ${outPath} (${xml.length} bytes, ${surahAyahs.length} ayahs, ${wordIdCounter - 1} words)`);
}
